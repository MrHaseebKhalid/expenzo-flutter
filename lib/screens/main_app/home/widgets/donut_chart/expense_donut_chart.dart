...
import 'package:expenzo/screens/main_app/home/widgets/donut_chart/expense_donut_chart_painter.dart';
...
class _ExpenseDonutChartState extends State<ExpenseDonutChart>
  ...
  void _initAnimations() {
    ...
    _pulseAnimation =
        TweenSequence<double>([
          TweenSequenceItem(
            tween: Tween<double>(begin: 1.0, end: widget.config.pulseScalePeak),
            weight: 50,
          ),
          TweenSequenceItem(
            tween: Tween<double>(begin: widget.config.pulseScalePeak, end: 1.0),
            weight: 50,
          ),
        ]).animate(
  ...
  void _onSegmentsChanged() {
    ...
    _recomputeGeometry(widget.segments);
    _fromSweepsRadians = _resizeList(
      currentSweeps,
      _targetSweepsRadians.length,
    );
  ...
  List<double> _currentAnimatedSweeps() {
    ...
    for (var i = 0; i < _targetSweepsRadians.length; i++) {
      ...
      if (!_loadComplete) {
        ...
        final from = i < _fromSweepsRadians.length ? _fromSweepsRadians[i] : to;
        final t = widget.config.valueChangeAnimationCurve.transform(
